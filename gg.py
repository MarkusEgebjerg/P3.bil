        ...
        for j in range(0, len(contour_centers)):
            ...
            if i != j and -afvig < (contour_centers[i][0] - contour_centers[j][0]) < afvig and contour_centers[i][1] < \
                ...
                cone_positions.append(
                    (contour_centers[i], contour_centers[j],
                     contour_centers[i][2]))  # list med to sammenhørende contours. højeste først
            if i != j and -afvig > (contour_centers[i][0] - contour_centers[j][0]) or afvig < (
                ...
                if p >= len(contour_centers) - 1:
                    ...
                    cone_positions.append((contour_centers[i], contour_centers[i][2]))

    for i in range(0, len(cone_positions)):
        Angle = cone_positions[i][0][0] / PxD - 43.5
        cv2.circle(color_array, (cone_positions[i][0]), 4, (255, 255, 255), -1)
        cone_distance = depth_frame.get_distance(int(cone_positions[i][0][0]), int(cone_positions[i][0][1]))
        cv2.putText(color_array, f"Angle: ({(round(Angle, 2))}, {(round(cone_distance, 2))}", cone_positions[i][0],
