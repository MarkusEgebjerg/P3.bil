...
while True:
    ...
    for i in range(2):
        ...
        for contour in list:
            ...
            if i == 1: contour_centers_y.append(current_center)
            else: contour_centers_B.append(current_center)
    ...
    for i in range(0, len(contour_centers_B)):
        p = 0
        for j in range(0, len(contour_centers_B)):

            if i != j and -afvig < (contour_centers_B[i][0] - contour_centers_B[j][0]) < afvig and contour_centers_B[i][1] < \
                    contour_centers_B[j][1]:
                cone_positions_B.append(
                    (contour_centers_B[i], contour_centers_B[j]))  # list med to sammenhørende contours. højeste først
            if i != j and -afvig > (contour_centers_B[i][0] - contour_centers_B[j][0]) or afvig < (
                    contour_centers_B[i][0] - contour_centers_B[j][0]):
                p = p + 1
                if p >= len(contour_centers_B) - 1:
                    cv2.circle(color_array, (contour_centers_B[i]), 4, (255, 0, 0), -1)


    for i in range(0, len(cone_positions_B)):
        Angle = cone_positions_B[i][0][0] / PxD - 43.5
        cv2.circle(color_array, (cone_positions_B[i][0]), 4, (255, 255, 255), -1)
        cone_distance = depth_frame.get_distance(int(cone_positions_B[i][0][0]), int(cone_positions_B[i][0][1]))
        cv2.putText(color_array, f"Angle: ({(round(Angle, 2))}, {(round(cone_distance, 2))}, Blue", cone_positions_B[i][0],
                    cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 255))

    for i in range(0, len(contour_centers_y)):
        p = 0
        for j in range(0, len(contour_centers_y)):

            if i != j and -afvig < (contour_centers_y[i][0] - contour_centers_y[j][0]) < afvig and contour_centers_y[i][
                1] < \
                    contour_centers_y[j][1]:
                cone_positions_y.append(
                    (contour_centers_y[i], contour_centers_y[j]))  # list med to sammenhørende contours. højeste først
            if i != j and -afvig > (contour_centers_y[i][0] - contour_centers_y[j][0]) or afvig < (
                    contour_centers_y[i][0] - contour_centers_y[j][0]):
                p = p + 1
                if p >= len(contour_centers_y) - 1:
                    cv2.circle(color_array, (contour_centers_y[i]), 4, (255, 0, 0), -1)

    for i in range(0, len(cone_positions_y)):
        Angle = cone_positions_B[i][0][0] / PxD - 43.5
        cv2.circle(color_array, (cone_positions_y[i][0]), 4, (255, 0, 255), -1)
        cone_distance = depth_frame.get_distance(int(cone_positions_y[i][0][0]), int(cone_positions_y[i][0][1]))
        cv2.putText(color_array, f"Angle: ({(round(Angle, 2))}, {(round(cone_distance, 2))}, Yellow ", cone_positions_y[i][0],
                    cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 255))
